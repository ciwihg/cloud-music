...
export default function(){
  ...
  function handleHotlistData(data){
    ...
    for (var i = 0; i < items.length; i++) {
      var temp=new itemFactory(items[i].id,items[i].name,items[i].alia,items[i].ar,items[i].al,items[i].privilege);
      newdatas.push(temp)
    }
    return newdatas;
  ...
  return class extends Component{
    ...
    render(){
       return (<Sglist datas={this.state.datas} highlight={this.props.highlight} ></Sglist>);
    }
